...
export default function TechnicalRound() {
    ...
    useEffect(() => {
        if (technicalQuestions.length === 0 && technologies.length > 0) {
            generateQuestions();
        }
    }, []);
    ...
    const generateQuestions = async () => {
        setLoading(true);
        try {
            const response = await api.generateTechnicalQuestions(technologies);
            setTechnicalQuestions(response.questions);
        } catch (error) {
    ...
    const handleSubmit = async () => {
        ...
        setLoading(true);
        try {
            const response = await api.evaluateTechnical(
                technicalQuestions[currentQuestion],
                recordedAudio
            );
            setEvaluation(response.evaluation);
            setTechnicalScore(currentQuestion, response.evaluation.score);
            setShowResult(true);
        } catch (error) {
    ...
    const handleNext = () => {
        setTechnicalAnswer(currentQuestion, recordedAudio);
    ...
    if (loading && technicalQuestions.length === 0) {
        return (
            <div className="flex items-center justify-center h-screen bg-gradient-to-br from-slate-900 to-slate-800">
                <div className="text-center">
                    <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-green-500 mx-auto mb-4"></div>
                    <p className="text-white text-lg">Generating technical questions...</p>
                </div>
            </div>
        );
    }
                ...
                <div className="mb-8">
                    <div className="flex justify-between items-center mb-2">
                        <h2 className="text-2xl font-bold text-white">
                            Technical Round: Question {currentQuestion + 1}/{technicalQuestions.length}
                        </h2>
                ...
                >
                    <div className="mb-8">
                        ...
                        <div className="bg-gray-900 p-4 rounded-lg">
                            <p className="text-gray-400 text-sm mb-2">Expected Key Points:</p>
                    ...
                    <div className="mb-8">
                        ...
                        <div className="bg-gray-900 p-6 rounded-lg border-2 border-dashed border-green-600">
                            <div className="flex flex-col items-center gap-4">
                                {isRecording ? (
                                    ...
                                    <>
                                        {recordedAudio ? (
                                            <>
                                                <p className="text-green-400 font-semibold">✓ Answer recorded</p>
                                                <button
                                            ...
                                            <>
                                                <FiMic className="text-4xl text-green-400" />
                    ...
                    {showResult && evaluation && (
                        <motion.div
                            ...
                            animate={{ opacity: 1, y: 0 }}
                            className="bg-gradient-to-r from-green-900 to-emerald-900 p-6 rounded-lg mb-8"
                        >
                            <h4 className="text-lg font-bold text-white mb-4">Evaluation Result</h4>
                            <div className="grid grid-cols-2 gap-4 mb-4">
                                <div className="bg-gray-800 p-3 rounded">
                                    <p className="text-gray-400 text-sm">Score</p>
                                    <p className="text-3xl font-bold text-green-400">{evaluation.score}</p>
                                </div>
                                <div className="bg-gray-800 p-3 rounded">
                                    <p className="text-gray-400 text-sm">Clarity</p>
                                    <p className="text-white text-sm">{evaluation.clarity}</p>
                                </div>
                            </div>
                            <div className="bg-gray-800 p-3 rounded mb-4">
                                <p className="text-gray-400 text-sm mb-2">Covered Key Points:</p>
                                <ul className="text-gray-300 text-sm space-y-1">
                                    {evaluation.coveredKeypoints?.map((point, idx) => (
                                        <li key={idx}>✓ {point}</li>
                                    ))}
                                </ul>
                            </div>
                            {evaluation.missingKeypoints?.length > 0 && (
                                <div className="bg-gray-800 p-3 rounded">
                                    <p className="text-gray-400 text-sm mb-2">Missing Key Points:</p>
                                    <ul className="text-gray-300 text-sm space-y-1">
                        ...
                        {!showResult ? (
                            <>
                                <button
                                    ...
                                    className="flex-1 bg-green-600 hover:bg-green-700 text-white font-bold py-3 px-4 rounded-lg disabled:opacity-50 transition-all"
                                >
                                    Submit Answer
                                </button>
                                <button
                            ...
                            >
                                {currentQuestion < technicalQuestions.length - 1 ? 'Next Question' : 'Finish Round'}
                            </button>
                        )}
                    </div>
                </motion.div>
